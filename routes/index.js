...
router.get('/table', isAuthenticated, async (req, res) => {
  ...
  console.log(debt)
  const tablerows = [];
  const totalPayedValue = [];
  const due = Debt.totalAmount / Debt.dues
  for (i = 0; i < debt.dues; i++) { 
    tablerows.push(i) 
    totalPayedValue.push(totalPayedValue[i-1]+due)
  }
  console.log(tablerows.length, totalPayedValue)
  res.render('table', { debt, tablerows })
})
